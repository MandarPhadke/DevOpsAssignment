...
def healthAndPerformanceOfServers():
    try:
        #getting the CPU percentage.
        perCPU = psutil.cpu_percent(interval=2)
        #if condition to check if CPU consuption is more than expected.
        if(perCPU > 79):
            print("Alert! CPU usage exceeds threshold: "+str(perCPU)+"%")
    except psutil.Error as error:
        stringerror = str(error)
        raise Exception("! Error: " + stringerror)

while True:
    healthAndPerformanceOfServers()